...
async def update_info():
    ...
    for guild in client.guilds:
        ...
        if next_rebase_secs <= 600:
            webhook = discord.Webhook.from_url(
                os.environ["https://discord.com/api/webhooks/907835961763844177/rnpYuMKOuVWfKK9Ng8pBpNu_bQNuOadxlilfeNWJATfVDuOj-tk6fskC-g7YxdNDNHCS"],
                adapter=discord.RequestsWebhookAdapter()
            )
            webhook.send(
                "Rebasing in a few minutes! (:deciduous_tree:, :deciduous_tree:) <@&904950683952902155>"
            )
